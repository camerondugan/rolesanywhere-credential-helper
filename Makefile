...
.PHONY: test-tpm-signer-minimal
test-tpm-signer-minimal: $(certsdir)/cert-bundle.pem $(certsdir)/tpm-sw-ec-secp384r1-key.pem $(certsdir)/tpm-sw-ec-secp384r1-sha256-cert.pem $(certsdir)/tpm-sw-ec-secp384r1-sha256-combo.pem
	$(STOP_SWTPM_TCP) 2>/dev/null || :
	$(START_SWTPM)
	go test ./... -run "TPM"
	$(STOP_SWTPM)
